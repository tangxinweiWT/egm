...
public class FileUtil {
    ...
    public static void upFile(MultipartFile file, String filePath, String fileName) {
        try {
            ...
            throw new InvalidParamException("程序错误！");
        }
    }
     ...
     * @param disPath
     */
    public static void unZip(MultipartFile file, String disPath) {
        // 清空并重建目标目录
        File outDirFile = new File(disPath);
        if (outDirFile.exists()) {
            FileUtil.deleteFileAll(outDirFile);
        }
        outDirFile.mkdirs();
        ...
        try {
            zipFile = new ZipInputStream(file.getInputStream(), StandardCharsets.UTF_8);
            File entryFile = null;
            String dirName = "";
            while((zipEntry = zipFile.getNextEntry()) != null) {
                ...
                // 如果有目录，去掉目录前缀
                entryFile = new File(disPath, zipEntry.getName().replaceAll(dirName, ""));
                @Cleanup OutputStream out = new FileOutputStream(entryFile);
                byte[] tmpBuffer = new byte[1024];
                int pos = -1;
                while((pos = zipFile.read(tmpBuffer)) > -1) {
                    out.write(tmpBuffer, 0, pos);
                }
            }
        } catch (IOException ioException) {
            throw new AppException("解压Zip失败"+ ioException.getMessage());
        }
    }
}
